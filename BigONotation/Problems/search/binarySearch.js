...
function binarySearchNumIndex(arr = [], target = 0, index = -1) {
  const len = arr.length;
  if (len === 0) return -1;
  if (len === 1) return arr[0] === target ? index : -1;
  const mid = Math.ceil(len / 2);
  ...
  if (target > left[left.length - 1]) {
    return binarySearchNumIndex(right, target);
  } else {
    return binarySearchNumIndex(left, target);
  }
}
...
t.that(
  binarySearchNumIndex(sorted50, 22),
  `binarySearchNumIndex(sorted50 , 22)`
).isEquals(21);

t.that(
  binarySearchNumIndex(sorted50, 100),
  `binarySearchNumIndex(sorted50 , 100)`
).isEquals(-1);
